"""NIST SP 800-53 Rev. 5 RAG pipeline.

We download the official NIST controls catalog (preferring the canonical OSCAL
JSON from NIST's GitHub since the legacy CSV download URLs change frequently),
chunk it per control (family + identifier + name + prose), embed locally with
sentence-transformers, store in a persistent ChromaDB collection, and retrieve
the most relevant control for a given risk query.

The prose returned is the *actual* NIST 800-53 control text - not a summary
generated by the LLM. The LLM later summarizes this retrieved text in plain
English. Grounding is enforced by passing the retrieved text to the LLM as
context with explicit "do not invent" instructions.
"""
...
# The OSCAL JSON catalog on NIST's official GitHub repo is the canonical,
# version-controlled, stable source. Prefer it over the CSV download URLs
# which NIST has changed several times.
NIST_OSCAL_JSON = (
    "https://raw.githubusercontent.com/usnistgov/oscal-content/main/"
    "nist.gov/SP800-53/rev5/json/NIST_SP-800-53_rev5_catalog.json"
)
# Legacy CSV URLs - kept as fallbacks in case the OSCAL repo is unreachable
NIST_URL_CSV1 = (
...
CACHE_TTL = 30 * 24 * 3600  # NIST controls rarely change; refresh monthly
...
def _oscal_json_to_df(catalog_json: dict) -> pd.DataFrame:
    """Flatten an OSCAL catalog JSON into the same column shape as the legacy CSV.

    Output columns: Control Identifier, Control (or Control Enhancement) Name,
    ...
    def _prose_for(control: dict, part_name: str) -> str:
        for p in control.get("parts", []) or []:
            if p.get("name") == part_name:
                ...
                if p.get("prose"):
                    bits.append(p["prose"])
                # Sub-parts (e.g. lettered statements) carry the actual control language
...
def fetch_nist_catalog(force: bool = False, cache_file: Path = CACHE_FILE) -> tuple[pd.DataFrame | None, str]:
    """Fetch the NIST 800-53 Rev. 5 control catalog. Tries OSCAL JSON first
    (canonical, GitHub-hosted, stable), then legacy CSV URLs, then bundled fallback.
    """
    ...
    # 1) Try OSCAL JSON from NIST's official GitHub (most reliable)
    try:
        r = requests.get(
    ...
    # 4) Bundled subset of authentic NIST 800-53 control text
...
def _normalise_columns(df: pd.DataFrame) -> pd.DataFrame:
    """NIST 800-53r5 control-catalog.csv columns are typically:

      Control Identifier, Control (or Control Enhancement) Name,
      Control Text, Discussion, Related Controls

    Be defensive about case / whitespace.
    """
    df = df.copy()
...
def build_chunks(df: pd.DataFrame) -> list[dict]:
    """One chunk per control. Each chunk carries id + searchable text + metadata."""
    ...
    for _, r in df.iterrows():
        ...
        # The text we embed contains everything searchable
...
class _KeywordFallbackIndex:
    """Lightweight TF-IDF-like retriever used only when chromadb/sentence-transformers
    cannot be loaded (e.g., a sandboxed environment without the embedding stack).
    Operates over the same NIST chunks. Real deployments use the Chroma path.
    """

    def __init__(self, chunks: list[dict]):
        self.chunks = chunks
        # Precompute lowered token sets per chunk
        self._toks = [
...
def get_or_build_index(force_rebuild: bool = False):
    """Returns a vector index populated with NIST 800-53 control chunks.

    Primary: ChromaDB + sentence-transformers embeddings.
    Fallback: keyword-overlap retriever over the same chunks (used when the
    embedding stack isn't available, e.g., minimal sandboxes).
    """
    ...
    # If forcing rebuild, drop collection
    if force_rebuild:
        try:
    ...
    # Add in batches (Chroma can be slow with huge batches)
    BATCH = 256
...
def build_query_for_risk(risk_row) -> str:
    ...
    # Add remediation hint if attached (we'll join remediation_guidance.csv later)
    rem_hint = risk_row.get("remediation_hint") or ""
    if rem_hint:
        parts.append(str(rem_hint))
    # Add intent words to bias toward remediation-style controls
    parts.append("remediation patch flaw vulnerability monitoring")
    return " | ".join(p for p in parts if p)


def retrieve_control(col, risk_row, n_results: int = 1):
    """Run a similarity search and return the top control's metadata."""
